...
### 新增
- **收藏夹拖拽归入**:文章卡片可拖拽到左侧文件夹,单篇快速归集(原生 HTML5 DnD,不用 Sortable)。
  详情见 [`docs/v0.4.25/changelog.md`](./docs/v0.4.25/changelog.md)。
- **收藏夹批量操作栏**:勾选多篇文章后显示「加入文件夹」按钮,弹窗单选目标夹,批量追加。
- **收藏夹子导航**:收藏页加 kb-subnav(仪表盘/全部文章/最近阅读/收藏夹/投稿)。
- **`POST /api/collections/{col_id}/articles`**:批量追加文章到收藏夹 API,双向同步 `source_ids`/`collection_ids`。

### 修复
- **活跃文件夹高亮失效**:原代码写 `dataset.col`(undefined),改为 `dataset.colId`(匹配 `data-col-id` 属性)。
- **批量按钮文案不一致**:知识库页「取消选择」改为「退出选择」,语义更清晰。
